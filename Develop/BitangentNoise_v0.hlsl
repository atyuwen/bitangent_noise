...
float3 BitangentNoise3D(float3 p)
{
	...
	i = i + 32768.5;
	uint2 hash0 = pcg3d16((uint3)i);
	uint2 hash1 = pcg3d16((uint3)(i + i1));
	uint2 hash2 = pcg3d16((uint3)(i + i2));
	uint2 hash3 = pcg3d16((uint3)(i + 1 ));

	float3 p00 = gradient3d(hash0.x); float3 p01 = gradient3d(hash0.y);
	float3 p10 = gradient3d(hash1.x); float3 p11 = gradient3d(hash1.y);
	float3 p20 = gradient3d(hash2.x); float3 p21 = gradient3d(hash2.y);
	float3 p30 = gradient3d(hash3.x); float3 p31 = gradient3d(hash3.y);
...
// 4D Bitangent noise. Approximately 165 instruction slots used.
// Assume p is in the range [-32768, 32767].
float3 BitangentNoise4D(float4 p)
{
	...
	i = i + 32768.5;
	uint2 hash0 = pcg4d16((uint4)i);
	uint2 hash1 = pcg4d16((uint4)(i + i1));
	uint2 hash2 = pcg4d16((uint4)(i + i2));
	uint2 hash3 = pcg4d16((uint4)(i + i3));
	uint2 hash4 = pcg4d16((uint4)(i + 1 ));

	float4 p00 = gradient4d(hash0.x); float4 p01 = gradient4d(hash0.y);
	float4 p10 = gradient4d(hash1.x); float4 p11 = gradient4d(hash1.y);
	float4 p20 = gradient4d(hash2.x); float4 p21 = gradient4d(hash2.y);
	float4 p30 = gradient4d(hash3.x); float4 p31 = gradient4d(hash3.y);
	float4 p40 = gradient4d(hash4.x); float4 p41 = gradient4d(hash4.y);
	...
	m0t = m0t * m0;
	m1t = m1t * m1;
	float3 temp0 = m0t * float3( dot( p00, x0 ), dot( p10, x1 ), dot( p20, x2 ) );
	float2 temp1 = m1t * float2( dot( p30, x3 ), dot( p40, x4 ) );
	float4 grad0 = -8.0 * (temp0.x * x0 + temp0.y * x1 + temp0.z * x2 + temp1.x * x3 + temp1.y * x4);
	grad0 += m04.x * p00 + m04.y * p10 + m04.z * p20 + m14.x * p30 + m14.y * p40;

	temp0 = m0t * float3( dot( p01, x0 ), dot( p11, x1 ), dot( p21, x2 ) );
	temp1 = m1t * float2( dot( p31, x3 ), dot( p41, x4 ) );
	float4 grad1 = -8.0 * (temp0.x * x0 + temp0.y * x1 + temp0.z * x2 + temp1.x * x3 + temp1.y * x4);
	grad1 += m04.x * p01 + m04.y * p11 + m04.z * p21 + m14.x * p31 + m14.y * p41;

	// The cross products of two gradients is divergence free.
	return cross(grad0.xyz, grad1.xyz) * 2992.1;
}
